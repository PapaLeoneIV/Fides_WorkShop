...
export async function updateExchange(bindKey: string, message: IOrderResponseDTO) {
    try {
...
export async function processOrderRequest(order_info: IOrderRequestDTO) {
    let PUBLISH_ORDER_BK = publisher.bindKeys.PublishBikeOrder;

    const orderExists = await orderRepository.read.checkExistance(order_info.order_id);
    if (orderExists) {
        console.log("[service] Order already exists");
        await updateExchange(PUBLISH_ORDER_BK, { id: order_info.order_id, status: status.DENIED });
        return;
    }
    ...
    if (hasSufficientBikes) {
        ...
        await updateExchange(PUBLISH_ORDER_BK, { id: updatedOrder.order_id, status: updatedOrder.renting_status });
        return;
    }
...
export async function processCancellationRequest(order_id: string) {
    let PUBLISH_ORDER_BK = publisher.bindKeys.PublishBikeOrder;
    let PUBLISH_ORDER_SAGA_BK = publisher.bindKeys.PublishbikeSAGAOrder;
    ...
    if (!orderExists) {
        ...
        await updateExchange(PUBLISH_ORDER_SAGA_BK, { id: order_id, status: status.DENIED });
        return;
    }
    ...
    if (order && order.renting_status !== status.APPROVED) {
        ...
        await updateExchange(PUBLISH_ORDER_SAGA_BK, { id: order.order_id, status: status.DENIED });
        return;
    }
